...
const { hasRevokedToken, insertRevokedToken } = require('./revokedTokens');
...
async function authenticateToken (req, res, next) {
  ...
  try {
    ...
    const userData = await findUser(req.user.sub);

    if (isRevoked || userData === null || userData.signature !== req.user.signature) {
